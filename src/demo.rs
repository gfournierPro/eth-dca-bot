...
async fn fetch_eur_usd_rate() -> Result<Decimal> {
    let client = reqwest::Client::new();
    
    // Fetch EUR/USD rate from Binance
    let url = "https://api.binance.com/api/v3/ticker/price?symbol=EURUSDC";
...
async fn analyze_market_conditions(market_data: LiveMarketData) {
    ...
    // Fetch real EUR/USD rate from Binance
    let eur_usd_rate = match fetch_eur_usd_rate().await {
        Ok(rate) => {
            println!("📡 Fetched live EUR/USD rate: {}", rate);
            rate
        }
        Err(e) => {
            println!("⚠️  Failed to fetch EUR/USD rate ({}), using fallback: 1.10", e);
            Decimal::new(110, 2) // 1.10 fallback
        }
    };
    ...
    println!("   EUR/USD Rate: {} (LIVE from Binance)", eur_usd_rate);
...
async fn fallback_demo_with_simulated_data() {
    ...
    // Try to fetch real EUR/USD rate, fallback to simulated if it fails
    let eur_usdc_rate = match fetch_eur_usd_rate().await {
        Ok(rate) => {
            println!("📡 Using live EUR/USD rate: {}", rate);
            rate
        }
        Err(_) => {
            println!("⚠️  Using simulated EUR/USD rate: 1.10");
            Decimal::new(110, 2) // 1.10 fallback
        }
    };
